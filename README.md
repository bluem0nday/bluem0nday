# Hi, I'm Matt MacQueen

I'm a product design leader and AI builder using Claude Code to close the gap between design and working software. Right now that means Claude workflows, custom Cowork skills, screenshot and calendaring utilities for iOS, agentic workflows, evals, and good old practical concept prototyping.

[macqueen.com](https://macqueen.com) · [LinkedIn](https://www.linkedin.com/in/mattmacqueen/)

## Current projects

- **[hold-times](https://github.com/bluem0nday/hold-times)** — a Claude skill that turns "let's find time to talk" into a booked call. Reads Google Calendar, places private holds, returns paste-ready times for email.
- **[pdf-redactor](https://github.com/bluem0nday/pdf-redactor)** — a Claude Code skill for true PDF redaction. Removes text from the file's content stream so recipients can't select, copy, or recover it.
- **pic-to-cal** (WIP) — turns a screenshot of an event (flyer, poster, Instagram story) into a calendar hold with the registration link attached. Built as a Claude skill, headed for an iOS prototype.
- **SOAR story coach** — a synthesizer and coach for Situation, Obstacle, Action, Result case studies. Turns raw project notes into outcome-driven narratives.
